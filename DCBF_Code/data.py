...
class SalObjDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        image, gt, depth, edge = self.resize(image, gt, depth, edge)
    ...
    def filter_files(self):
        ...
        for img_path, gt_path, depth_path, edge_path in zip(self.images, self.gts, self.depths, self.edges):
            if len(self.images) == len(self.gts) == len(self.edges) == len(self.depths):
    ...
    def resize(self, img, gt, depth, edge):
        ...
        w, h = self.trainsize, self.trainsize
...
def get_loader(image_root, gt_root, depth_root, edge_root,batchsize, trainsize, shuffle=True, num_workers=12, pin_memory=True):
...
class calibration_dataset:
    ...
    def __getitem__(self,index):
        ...
        # data augment
        image, _, depth, _ = cv_random_flip(image, depth, depth, depth)
        image, _, depth, _ = randomCrop(image, depth, depth, depth)
        image, _, depth, _ = randomRotation(image, depth, depth, depth)
        image = colorEnhance(image)
    ...
    def resize(self, img, depth):
        # assert img.size == gt.size and gt.size == depth.size
        w, h = self.testsize, self.testsize
